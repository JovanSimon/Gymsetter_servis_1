...
public class ClientServiceImpl implements ClientService {
    ...
    @Override
    public Integer incriseNumOfTermins(Long id) {
        Client client = clientRepository.findById(id).orElseThrow(() ->
                new NotFoundException("Client with id: " + id + " not found.")
        );

        client.setNumOfTermins(client.getNumOfTermins() + 1);
        clientRepository.save(client);
        return client.getNumOfTermins();
    }
    ...
    @Override
    public Integer decrement(Long id) {
        Client client = clientRepository.findById(id).orElseThrow(() ->
                new NotFoundException("Client with id: " + id + " not found.")
        );
        ...
        client.setNumOfTermins(client.getNumOfTermins() - 1);
        clientRepository.save(client);
        return client.getNumOfTermins() - 1;
    }
