...
public class ClientController {
    ...
    public ClientController(ClientService clientService, TokenService tokenService, JmsTemplate jmsTemplate,
                            MessageHelper messageHelper, ClientRepository clientRepository, NotificationRepository notificationRepository,
                            NotificationTypeRepository notificationTypeRepository, @Value("${destination.notify}") String orderDestination){
    ...
    @GetMapping("/mail/{id}")
    public ResponseEntity<String> findEmail(@PathVariable("id") Long id){
        return new ResponseEntity<>(clientRepository.findClientById(id).getEmail(), HttpStatus.OK);
    }

    @GetMapping("/num-of-termins/{id}")
    public ResponseEntity<Integer> getNumOfTermins(@PathVariable("id") Long id){
        return new ResponseEntity<>(clientRepository.findClientById(id).getNumOfTermins(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ClientDto> add(@RequestBody ClientCreateDto clientCreateDto){
        ...
        jmsTemplate.convertAndSend(orderDestination,
                messageHelper.createTextMessage(new NotiDto(clientCreateDto.getEmail(), "REGISTER_TYPE", tmp.getId())));
    ...
    public ResponseEntity<Integer> decrement(@PathVariable("id") Long id){
        return new ResponseEntity<>(clientService.decrement(id), HttpStatus.OK);
    }
    ...
    @GetMapping("/incirise/{id}")
    public ResponseEntity<Integer> incriseNumOfTermins(@PathVariable("id") Long id) {
        return new ResponseEntity<>(clientService.incriseNumOfTermins(id), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ClientDto> updatePassword(@RequestHeader(name = "Authorization") String token, @RequestBody ClientUpdatePasswordDto clientUpdatePasswordDto) {
        ...
        if (claims != null) {
            ...
            jmsTemplate.convertAndSend(orderDestination,
                    messageHelper.createTextMessage(new NotiDto(email, "PASSWORD_TYPE", userId)));
